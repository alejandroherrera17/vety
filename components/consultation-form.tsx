...
export function ConsultationForm({ petId }: { petId: string }) {
  ...
  function onSubmit(values: ConsultationInput) {
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        toast.error(result.error ?? "Could not save consultation");
      }
    });
  }
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4">
      ...
      <Field label="Symptoms" error={form.formState.errors.symptoms?.message}>
        <Textarea
          autoFocus
          placeholder="Clinical signs, duration, appetite, behavior..."
          className="min-h-40 text-base"
          {...form.register("symptoms")}
        />
      </Field>
      <Field label="Diagnosis" error={form.formState.errors.diagnosis?.message}>
        <Textarea className="min-h-36 text-base" {...form.register("diagnosis")} />
      </Field>
      <Field label="Treatment" error={form.formState.errors.treatment?.message}>
        <Textarea className="min-h-36 text-base" {...form.register("treatment")} />
      </Field>
      <Field label="Observations" error={form.formState.errors.observations?.message}>
