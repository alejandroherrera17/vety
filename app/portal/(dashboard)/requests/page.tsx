...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case "approved":
      return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20"><CheckCircle2 className="h-3.5 w-3.5" /> Aprobada</span>;
    case "rejected":
...
export default async function PortalRequestsPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Mis Solicitudes de Citas</h1>
      ...
      {requests.length === 0 ? (
        <div className="bg-white dark:bg-surface border border-slate-200 dark:border-border rounded-xl p-12 text-center shadow-sm">
          ...
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">No tienes solicitudes pendientes</h3>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center">
                    ...
                    <span className="sm:ml-3 text-emerald-600 dark:text-emerald-400 font-medium flex items-center mt-1 sm:mt-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 dark:bg-surface-hover rounded-lg p-4 border border-slate-100 dark:border-border">
                <div className="space-y-4">
                  ...
                  {request.requestedVeterinarian && (
                    <div>
                      ...
                      <p className="font-medium text-slate-900 dark:text-white">Dr/a. {request.requestedVeterinarian.name}</p>
                    </div>
                  )}
                ...
                <div className="space-y-4 md:border-l md:border-slate-200 dark:border-slate-800 md:pl-6">
                  {request.proposedStart && (
                    <div>
                      <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1">Fecha Confirmada/Propuesta</p>
                      <p className="font-medium text-emerald-700 dark:text-emerald-300 flex items-center">
